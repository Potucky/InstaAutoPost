...
CLAUDE_TIMEOUT = int(os.environ.get("POTUCKY_CLAUDE_TIMEOUT_SECONDS", 600))
CODEX_TIMEOUT = int(os.environ.get("POTUCKY_CODEX_TIMEOUT_SECONDS", 300))
...
1. Run: git status --short
   Verify: only files in the ALLOWED FILES list, plus the KNOWN ORCHESTRATION
   ARTIFACTS listed below, appear as modified or added. Any file outside both
   lists is a scope violation and a FAIL blocker.
...
## KNOWN ORCHESTRATION ARTIFACTS (expected — not scope violations)
The following files may appear in `git status --short` as modified or untracked.
They are managed by potucky_orchestrator.py automatically — NOT written by the
Claude executor. Do NOT treat them as scope violations or FAIL blockers.

  - scripts/potucky_agent_tasks.json
    Modified by: orchestrator run-next / auto-execute (updates "status" field only).
    Verify: the diff shows only a "status" value change. Any other diff IS a blocker.

  - scripts/potucky_agent_prompts/{task_id}_claude_executor_prompt.md
    Generated by: orchestrator auto-execute / prepare-prompts. Expected untracked.
    Verify: file contains no secrets or sensitive values.

  - scripts/potucky_agent_prompts/{task_id}_codex_review_prompt.md
    Generated by: orchestrator auto-execute / prepare-prompts. This prompt file.
    Verify: file contains no secrets or sensitive values.

  - scripts/potucky_agent_prompts/logs/ (*.log files — gitignored)
    Generated by: orchestrator auto-execute (Claude/Codex CLI output capture).
    These are gitignored; they should not appear in `git status --short`.
...
def cmd_doctor():
    ...
    print()
    print("  Timeouts (override via env vars):")
    print(f"    POTUCKY_CLAUDE_TIMEOUT_SECONDS  {CLAUDE_TIMEOUT}s")
    print(f"    POTUCKY_CODEX_TIMEOUT_SECONDS   {CODEX_TIMEOUT}s")
    print()
    print("=" * 62)

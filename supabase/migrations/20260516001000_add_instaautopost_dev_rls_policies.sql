...
-- Migration: 20260516001000_add_instaautopost_dev_rls_policies.sql
--
-- Grants all authenticated users full access to all three tables.
-- These policies target the `authenticated` role — the UI requires an active
-- Supabase session (logged-in user) to read or write any data via the anon key.
--
-- The publisher worker uses the service_role key and bypasses RLS entirely.
-- Worker dry-run mode can be tested without a UI session if seed rows exist
-- and SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY are configured.
--
